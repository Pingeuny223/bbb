...
| `min_free_ratio` | | 전체 좌석 대비 최소 잔여 **비율**(0~1). 기본 0(조건 없음) |
...
### `min_free_ratio` — 좋은 자리가 남았을 때만

좌석이 거의 다 나간 회차에서 나오는 취소표는 맨 앞줄이나 구석입니다.
숫자상 4석이 있어도 여럿이 나란히 앉을 수는 없습니다.

`min_free_ratio` 는 **전체 좌석 대비 최소 잔여 비율**입니다(퍼센트가 아니라 비율 — 50%는 `0.5`).
`min_seats` 와 비율 중 **더 엄격한 쪽**이 그 회차의 기준이 됩니다.

```yaml
# 특별관: 자리가 얼마 없어도 볼 만하므로 비율 조건을 걸지 않는다
- name: "영등포 IMAX"
  screen_types: ["IMAX"]
  min_seats: 4            # 387석 중 4석만 남아도 알림

# 일반관: 좋은 자리가 아니면 갈 이유가 없다
- name: "영등포 5관"
  screen_types: ["5관"]
  min_seats: 4
  min_free_ratio: 0.5     # 320석 중 160석 이상 남아야 알림
```

비율을 걸면 사실상 **신규 편성이나 대량 취소일 때만** 알림이 옵니다.
자잘한 취소표를 걸러내는 것이 목적이므로 의도된 동작입니다.

### 좌석 수만 알 수 있습니다 ⚠️

CGV API는 **잔여 좌석 "개수"만** 주고 어느 자리인지는 주지 않습니다.
`min_seats: 4` 알림이 와도 **네 자리가 붙어 있다는 보장은 없습니다.**
여러 명이 함께 볼 계획이라면, 취소표보다 아래의 신규 편성 알림이 현실적인 기회입니다.

### 생존 신고 (heartbeat)

`heartbeat_hours: 12` 로 두면 12시간마다 한 번 "감시 정상 동작 중" 알림을 보냅니다
(하루 2회). `0` 이면 끕니다.

알림이 조용한 것이 *정상*인지 *죽은 것*인지 구분하기 위한 장치입니다.
기왕 보내는 김에 현재 잔여석 현황도 함께 실어, 한 번에 상황이 파악되게 했습니다.
조건에 맞는 회차가 하나도 없으면 그 사실을 경고로 알려줍니다
(config 의 영화명이나 지점 오타를 잡아냅니다).

### 편성 등록 알림 (예매 오픈 전)

`notify_on_listed: true` 로 두면, 편성만 올라오고 **아직 예매가 열리지 않은
회차(0석)** 도 알립니다. 그 시점엔 예매할 수 없지만 "이 회차가 생긴다"는
예고로 쓸 수 있습니다.

실제로 예매가 열리면(0석 → N석) **'좌석 발생' 알림이 별도로 한 번 더** 갑니다.
등록 알림은 쿨다운을 소모하지 않기 때문입니다 — 소모하게 두면 몇 분 뒤의
진짜 알림이 쿨다운에 걸려 통째로 묻힙니다.

0석일 때만 '등록'으로 봅니다. 새로 보이는데 좌석이 조금 있는 회차는
그냥 조건에 못 미치는 회차이므로 알리지 않습니다.

### 좌석 감소 경고 (`alert_below_ratio`)

잔여석이 **전체의 정한 비율 아래로 내려오는 순간** 한 번 알립니다.
`0.65` 면 65%입니다(퍼센트가 아니라 비율). `0` 이면 끕니다.

**회차당 딱 한 번만** 보냅니다. 기준선 근처에서 좌석이 오르내려도
(126 → 128 → 124) 같은 경고가 반복되지 않습니다.
이 표시는 state 에 저장되어 재시작 후에도 유지됩니다.

'아래로 내려오는 순간'을 잡는 것이지 '아래에 있음'을 잡는 게 아닙니다.
처음 봤을 때 이미 기준선 아래인 회차는 계속 줄어도 조용합니다.

> 모든 감소를 알리면 노이즈가 감당이 안 됩니다. 실측으로 320석 관이 약
> 17분에 1석씩 빠졌는데, 3분 간격 폴링에서는 회차당 하루 80건이 넘습니다.
> 그래서 임계선 교차 방식만 제공합니다.

### 감시 기간이 지나면

`config.yaml` 의 모든 `date_range` 가 과거가 되면 CGV 조회를 **아예 건너뜁니다.**
잡을 것이 없는데 5분마다 두드릴 이유가 없기 때문입니다.
대신 하루 한 번 "감시 기간 종료" 알림을 보냅니다.

계속 쓰려면 `date_range` 를 새 날짜로 바꾸고, 당분간 안 쓸 거면
Actions 탭에서 워크플로를 비활성화하세요 (⋯ → *Disable workflow*).
...
`notify_on_first_seen: true` 로 두면 **이전에 없던 회차가 나타났을 때도** 알립니다.
극장이 새 상영관 편성을 열었다는 뜻이므로, 취소표보다 이쪽이 더 중요한 경우가 많습니다.

이 옵션은 **이전 state가 있을 때만** 동작합니다.
첫 실행이나 캐시 유실 시에는 모든 회차가 '신규'로 보이므로 알림을 억제하고 기준선만 기록합니다.

> ⚠️ **규칙을 넓힐 때 주의.** `config.yaml` 에서 상영관을 추가하거나 날짜·시간 범위를
> 늘리면, 새로 범위에 들어온 기존 회차들이 '신규 편성'으로 오인돼 알림이 쏟아집니다.
> 이럴 때는 `watch.yml` 의 캐시 키 프리픽스(`cgv-state-v3-`)에서 숫자를 올리세요.
> 다음 실행이 콜드 스타트가 되어 기준선을 새로 잡습니다.
> (범위를 *좁히는* 것은 안전합니다.)
...
**GitHub Actions의 `schedule` 은 정시를 보장하지 않습니다. 생각보다 훨씬 심합니다.**

이 레포에서 `*/5`(5분 간격)로 걸고 실측한 결과입니다:
...
5분마다 트리거를 걸어도 GitHub이 대부분을 버립니다. 무료 public 레포에서 흔한 일이고,
cron 표현식을 바꿔도 근본적으로는 GitHub의 부하 정책이라 해결되지 않습니다.

### 그래서: 자주 깨우지 않고, 한 번 깨어나면 오래 돈다

```yaml
polling:
  run_duration_minutes: 50   # 한 번 실행에서 50분간 계속 감시
  round_interval_sec: [170, 190]  # 약 3분 간격 → 약 17라운드
```

트리거 빈도로 주기를 만드는 대신, **실행 하나가 오래 살아 있으면서** 주기를 만듭니다.
스케줄이 얼마나 밀리든 감지 간격은 3분으로 유지됩니다.

공백이 생기지 않는 이유는 `concurrency` 덕분입니다. 실행 중에 들어온 트리거는
대기 슬롯 하나를 차지하고, 실행이 끝나는 즉시 이어받습니다.
그래서 cron 은 `*/5` 로 촘촘히 걸어두는 편이 낫습니다 — 대부분 버려지지만
하나만 대기 슬롯에 남아 있으면 됩니다.

- `run_duration_minutes: 0` 이면 예전처럼 `rounds_per_run` 만큼만 돌고 끝냅니다.
- job `timeout-minutes` 는 `run_duration_minutes` 보다 넉넉히 크게 두세요(현재 60).
- GitHub 의 job 최대 실행 시간은 6시간입니다.
- 오래 도는 실행이 중간에 죽어도 관측을 잃지 않도록 **라운드마다 state를 저장**합니다.
- 레포에 60일간 활동이 없으면 스케줄이 자동 비활성화됩니다(이메일로 안내가 옵니다).

**정확한 주기가 꼭 필요하다면** Actions는 맞지 않습니다.
상시 켜져 있는 서버나 라즈베리파이에서 돌리는 편이 낫고, 코드는 그대로 쓸 수 있습니다.
